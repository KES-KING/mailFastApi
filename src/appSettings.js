...
const SETTING_DEFINITIONS = Object.freeze([
  group("Runtime", [
    ...
    select("WORKER_RESOURCE_MODE", "Worker resource mode", ["fixed", "max"], "fixed", "core", "Max mode uses aggressive auto concurrency when WORKER_CONCURRENCY is empty/auto."),
    integer("WORKER_CONCURRENCY", "Worker concurrency", 2, 1, 10000, "core", "Parallel worker jobs per process."),
  ...
  group("Delivery and suppression", [
    ...
    select("DLQ_AUTO_RETRY_ENABLED", "Auto retry failed mail", ["true", "false"], "true", "core", "Worker automatically requeues pending failed/dead-lettered jobs."),
    integer("DLQ_AUTO_RETRY_MAX_ATTEMPTS", "Auto retry max attempts", 3, 0, 100, "core", "Maximum automatic DLQ requeue attempts before final error."),
    integer("DLQ_AUTO_RETRY_INTERVAL_MS", "Auto retry interval ms", 30000, 1000, 3600000, "core", "Background DLQ retry scan interval."),
    integer("DLQ_AUTO_RETRY_BATCH_SIZE", "Auto retry batch size", 100, 1, 5000, "core", "Maximum failed jobs requeued per scan."),
  ]),
